...
def calculate_irr(cashflows):
    ...
    x = 0.1
    for i in range(100):
        fx = sum(cf / ((1 + x) ** t) for t, cf in enumerate(cashflows))
        if abs(fx) < 0.01:
...
def calculate_scenario(growth_rate):
    ...
    for year in range(6):
        if year == 0:
            years_data.append({
                'Year': 0, 'Revenue': current_revenue, 'COGS': 0, 'Labor': 0,
                'Rent': 0, 'Other OpEx': 0, 'EBITDA': 0, 'Debt Service': 0,
                'Cash Flow': -down_payment, 'Cumulative CF': cumulative_cf
            })
        else:
            ...
            years_data.append({
                'Year': year, 'Revenue': revenue, 'COGS': cogs, 'Labor': labor,
                'Rent': rent, 'Other OpEx': other_opex, 'EBITDA': ebitda,
                'Debt Service': annual_debt_service, 'Cash Flow': cash_flow,
...
with tab1:
    ...
    fig = go.Figure()
    for df, name, color in [(pessimistic_df, 'Pessimistic', 'red'), (base_df, 'Base', 'blue'), (optimistic_df, 'Optimistic', 'green')]:
        filtered = df[df['Year'] > 0]
        fig.add_trace(go.Scatter(x=filtered['Year'], y=filtered['Revenue'], name=name, line=dict(color=color, width=3), mode='lines+markers'))
    fig.update_layout(xaxis_title="Year", yaxis_title="Revenue ($)", height=400, hovermode='x unified', yaxis=dict(tickformat='$,.0f'))
...
with tab2:
    ...
    for df, name, color in [(pessimistic_df, 'Pessimistic', 'red'), (base_df, 'Base', 'blue'), (optimistic_df, 'Optimistic', 'green')]:
        filtered = df[df['Year'] > 0]
        fig.add_trace(go.Bar(x=filtered['Year'], y=filtered['Cash Flow'], name=name, marker_color=color))
    fig.update_layout(xaxis_title="Year", yaxis_title="Cash Flow ($)", barmode='group', height=400, yaxis=dict(tickformat='$,.0f'))
    st.plotly_chart(fig, use_container_width=True)
    
    st.subheader("Cumulative Cash Flow")
    fig2 = go.Figure()
    for df, name, color in [(pessimistic_df, 'Pessimistic', 'red'), (base_df, 'Base', 'blue'), (optimistic_df, 'Optimistic', 'green')]:
        fig2.add_trace(go.Scatter(x=df['Year'], y=df['Cumulative CF'], name=name, line=dict(color=color, width=3), mode='lines+markers'))
    fig2.add_hline(y=0, line_dash="dash", line_color="gray", annotation_text="Break Even")
    fig2.update_layout(xaxis_title="Year", yaxis_title="Cumulative Cash Flow ($)", height=400, yaxis=dict(tickformat='$,.0f'))
...
with tab4:
    ...
    with col1:
        ...
        st.download_button(label="📄 Download Base Case CSV", data=csv, file_name="coffee_shop_base_case.csv", mime="text/csv")
    with col2:
        summary_data = {
            'Metric': ['Purchase Price', 'Down Payment', 'Loan Amount', 'Interest Rate', 'Loan Term', 'Pessimistic IRR', 'Base IRR', 'Optimistic IRR'],
            'Value': [f"${purchase_price:,}", f"${down_payment:,.0f}", f"${purchase_price - down_payment:,}", f"{interest_rate}%", f"{loan_term} years", f"{pessimistic_irr}%", f"{base_irr}%", f"{optimistic_irr}%"]
        }
        summary_df = pd.DataFrame(summary_data)
        csv_summary = summary_df.to_csv(index=False)
        st.download_button(label="📊 Download Summary", data=csv_summary, file_name="investment_summary.csv", mime="text/csv")
